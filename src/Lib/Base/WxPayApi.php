...
class WxPayApi
{
	 ...
	 * @return 成功时返回，其他抛异常
	 */
	public static function unifiedOrder(WxPayConfigInterface $config, $inputObj, $timeOut = 6)
	{
	 ...
	 * @return 成功时返回，其他抛异常
	 */
	public static function orderQuery(WxPayConfigInterface $config, $inputObj, $timeOut = 6)
	{
	 ...
	 * @return 成功时返回，其他抛异常
	 */
	public static function closeOrder(WxPayConfigInterface $config, $inputObj, $timeOut = 6)
	{
		$url = "https://api.mch.weixin.qq.com/pay/closeorder";
		//检测必填参数
		if(!$inputObj->IsOut_trade_noSet()) {
			throw new WxException("订单查询接口中，out_trade_no必填！");
		}
		$inputObj->SetAppid($config->GetAppId());//公众账号ID
		$inputObj->SetMch_id($config->GetMerchantId());//商户号
		$inputObj->SetNonce_str(self::getNonceStr());//随机字符串
	 ...
	 * @return 成功时返回，其他抛异常
	 */
	public static function refund(WxPayConfigInterface $config, $inputObj, $timeOut = 6)
	{
	 ...
	 * @return 成功时返回，其他抛异常
	 */
	public static function refundQuery(WxPayConfigInterface $config, $inputObj, $timeOut = 6)
	{
	 ...
	 * @return 成功时返回，其他抛异常
	 */
	public static function downloadBill(WxPayConfigInterface $config, $inputObj, $timeOut = 6)
	{
	 ...
	 * @param int $timeOut
	 */
	public static function micropay(WxPayConfigInterface $config, $inputObj, $timeOut = 10)
	{
	...
	public static function bizpayurl($config, $inputObj, $timeOut = 6)
	{
		if(!$inputObj->IsProduct_idSet()){
			throw new WxException("生成二维码，缺少必填参数product_id！");
		}
	...
	public static function shorturl($config, $inputObj, $timeOut = 6)
	{
 	 ...
 	 */
	public static function notify(WxPayConfigInterface $config, $callback, &$msg)
	{
	 ...
	 * @param array $data
	 */
	private static function reportCostTime(WxPayConfigInterface $config, $url, $startTimeStamp, $data)
	{
		//如果不需要上报数据
		$reportLevenl = $config->GetReportLevel();
		if($reportLevenl == 0){
			return;
		} 
		//如果仅失败上报
	 ...
	 * @throws WxException
	 */
	private static function postXmlCurl(WxPayConfigInterface $config, $xml, $url, $useCert = false, $second = 30)
	{		
		$ch = curl_init();
